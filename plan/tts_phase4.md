...
*   **Warning**: "You are about to listen to a whole chapter (~20k chars). Proceed with Cloud Voice?" (Optional toggle in settings).
    *   *Status:* Implemented via `ReaderView` check (threshold 5000 chars) and custom `Dialog`. Toggle added to TTS settings.
...
1.  **Cost Controls (Completed)**:
    ...
    *   Add custom warning dialog and toggle for large text blocks in `ReaderView`.
