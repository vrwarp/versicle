...
| 13 | `src/lib/search.test.ts` (SearchClient unit suite — indexing dedupe, worker wiring) | `src/domains/search/SearchSession.test.ts` | `indexes provided sections and searches with per-occurrence results` / `dedupes concurrent index calls for the same book (one extraction, one task)` | ✅ deleted with `lib/search.ts` at the reader adoption |
| 14 | `src/test/search-client.repro.test.ts` (concurrent-indexing progress broadcast repro) | `src/domains/search/SearchSession.test.ts` | `dedupes concurrent index calls…` (shared-task semantics) + `dispose() rejects in-flight indexing (SEARCH_SESSION_DISPOSED) and clears caches` — the old `terminate()` leak this repro guarded is structurally gone | ✅ deleted with `lib/search.ts` at the reader adoption |
| 15 | `src/components/reader/panels/SearchPanel.test.tsx` — REWRITTEN (was a searchClient-mock suite) onto the REAL SearchSession + in-process engine; adds reprocess-fallback, truncation-note and exact-occurrence-navigation cases | — | — | ✅ rewritten at the reader adoption |
