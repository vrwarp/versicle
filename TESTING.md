...
### Performance / jank instrumentation specs
...
| Spec | Measures |
| --- | --- |
| `test_perf_baseline.spec.ts` | The LOAD path: cold boot, demo import, book open, page turns, warm reload — step wall-clock plus boot User Timing, navigation/paint timing, slowest resources, rAF frame gaps. (Its second test IS a real assertion: the 3s service-worker boot-cliff guard.) |
| `test_perf_jank.spec.ts` | The INTERACTION path: the Chinese pinyin overlay under 4x CPU throttling (CDP) on a large zh book — enabling pinyin, page turns, pinyin-size change, scrolled-mode switch, scroll-settle remeasures. Reports per step: long tasks (with attribution), rAF gaps >33ms and the blocking time they imply, `chinese:process-section` pass durations, and overlay span counts. |

`test_perf_jank.spec.ts` runs on `verification/test_chinese_large.epub`
(~13.5k Han characters over 3 chapters), regenerated by
`node verification/create_perf_chinese_epub.cjs`. It is deliberately a
SEPARATE fixture from `test_chinese.epub`, which stays byte-identical
