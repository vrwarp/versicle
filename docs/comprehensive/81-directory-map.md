...
| [`embeddingBackfill.ts`](../../src/app/boot/embeddingBackfill.ts) | `backgroundTasks` | Trickles every locally present book through the `EmbeddingIndexer` on the background quota lane during idle time, so the whole on-device library is semantic-searchable without first being opened. Runs only when the default-OFF `preEmbedLibrary` opt-in is on (its consent for bulk background egress), the client is configured, and this device's heartbeat is recent; every embed is `{interactive:false, lane:'bg'}` and honors the cross-device daily-request budget. |
