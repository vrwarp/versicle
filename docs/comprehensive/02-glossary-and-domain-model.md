...
Cross-references: [Architecture overview](10-architecture-overview.md) | [State management & CRDT](13-state-management-crdt.md) | [Storage gateway](20-storage-gateway.md) | [Bootstrap & lifecycle](14-bootstrap-and-lifecycle.md) | [TTS engine](32-domain-audio-tts-engine.md) | [Library domain](37-domain-library.md)
...
The IndexedDB database is named `EpubLibraryDB` ([src/data/schema.ts](../../src/data/schema.ts)) and is currently at version **26** (`DB_VERSION = 26`).
...
- **CACHE** (`cache_*`) — ephemeral and regenerable derived data. All cache stores can be rebuilt from the static domain.
- **APP** (`checkpoints`, `sync_log`, `app_metadata`, `flight_snapshots`) — sync infrastructure and diagnostics.
...
The `cfi` field is never produced by the search engine (which never touches the DOM). It is resolved lazily on demand via `resolveResultCfi` when the user clicks a result. The indexed corpus lives in `cache_search_text` (IDB v26, one row per book keyed by `bookId`).

---

## 13. Diagnostics — FlightSnapshot
...
## 15. Relationships — Full Domain ER Diagram
...
### 16.1 The Static / User Data Boundary
...
### 16.2 Progress Fallback Priority
...
### 16.3 CFI Null Safety in TTS Queue Items
...
### 16.4 `__schemaVersion` as Poison Pill
...
### 16.5 Reading List / Inventory Dual Keying
...
## 17. Complete Sequence: Ingest to First Playback
