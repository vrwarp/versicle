...
- [`src/domains/genai/errors.ts`](../../src/domains/genai/errors.ts) — `GenAINotConfiguredError`, `GenAIInvalidResponseError`, `GenAIHttpError` (retryable at 429 / ≥500).
- [`src/domains/genai/embedding/errors.ts`](../../src/domains/genai/embedding/errors.ts) — `EmbeddingNotConfiguredError` (code `GENAI_EMBEDDING_NOT_CONFIGURED`), thrown by the embedding holder's NOT-CONFIGURED default so a stray import degrades exactly like a missing API key rather than crashing.
