...
The model-rotation path treats the same backpressure as retryable. [`src/domains/google/genai/GeminiClient.ts`](../../src/domains/google/genai/GeminiClient.ts) rotates to the next model when `isRetryableForRotation(error)` is true, and that predicate ([`src/domains/google/genai/errors.ts`](../../src/domains/google/genai/errors.ts)) returns true for a server 429 (`GenAIHttpError.status === 429`), an unusable model (`isModelUnavailable`), **and** a pre-network `NetRateLimitedError` — a sibling model's 429 sets the governor cooldown, so this model's gateway `acquire` backpressures before the network, and rotation moves on to a model that may still have budget. A 429 the server does send is fed back to the governor via `recordCooldown(retryAfterMs(response, …))` and then re-thrown so the rotation loop still sees it.

`isModelUnavailable` covers the case where the model — not the request — is the problem: a **404** (the endpoint is `/models/<id>:generateContent`, so the only thing that can be Not Found is the model), or a 400 whose API `status` is `FAILED_PRECONDITION` (not enabled for this project/region/tier). It matters because Google retires models on its own schedule and a retired one answers 404 rather than 429; without this arm the loop would rethrow on the spot and strand every model below it in `GENAI_ROTATION_MODELS`, including the high-daily-quota ones that carry most of the day. A bare 400 `INVALID_ARGUMENT` is deliberately excluded — a malformed prompt or schema breach fails identically on every model, so rotating would turn one bad request into a round trip per model.
