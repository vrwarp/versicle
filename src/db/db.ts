...
export interface EpubLibraryDB extends DBSchema {
  ...
  // --- DOMAIN 2: USER (LEGACY / MIGRATED TO YJS) ---
  /**
   * @deprecated Migrated to Yjs `books` map.
   * Kept for migration source only.
   */
  user_inventory: {
    key: string;
  ...
  /**
   * @deprecated Migrated to Yjs `readingList`.
   */
  user_reading_list: {
    key: string;
  ...
  /**
   * @deprecated Migrated to Yjs `readingState` (completedRanges, currentPosition).
   */
  user_progress: {
    key: string;
    value: UserProgress;
