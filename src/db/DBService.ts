...
class DBService {
  ...
  async ingestBook(data: BookExtractionData): Promise<void> {
    try {
      ...
      // Phase 2: user_inventory, user_progress, user_reading_list managed by Yjs stores.
      // We only write static data and cache here.
      // Exception: user_overrides is still synced via legacy path for now.
