...
export class DBService {
  ...
  // Map stores { timeout, resolve } to ensure we can resolve superseded promises
  private pendingProgressUpdates: Map<string, { timeout: NodeJS.Timeout; resolve: () => void }> = new Map();
  ...
  async saveProgress(id: string, cfi: string, progress: number): Promise<void> {
    ...
    if (this.pendingProgressUpdates.has(id)) {
      const pending = this.pendingProgressUpdates.get(id)!;
      clearTimeout(pending.timeout);
      // Resolve the previous promise so it doesn't hang
      pending.resolve();
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(async () => {
        ...
        try {
          ...
          if (book) {
            book.currentCfi = cfi; // Update reading position
      ...
      this.pendingProgressUpdates.set(id, { timeout, resolve });
    });
  }
