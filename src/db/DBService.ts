...
class DBService {
   ...
   * Note: Does NOT fetch cover blobs to keep the list view lightweight.
   *
  ...
  async getBook(id: string): Promise<{ metadata: BookMetadata | undefined; file: Blob | ArrayBuffer | undefined }> {
    try {
      const db = await this.getDB();
      const tx = db.transaction(['static_books', 'static_book_sources', 'user_book_states', 'static_files', 'cache_covers'], 'readonly');
      ...
      const coverBlob = await tx.objectStore('cache_covers').get(id);
      ...
      const metadata: BookMetadata = {
          ...
          ...(source || {}),
          ...(state || {}),
          coverBlob // Attach cover blob from cache
      };
  ...
  async getBookMetadata(id: string): Promise<BookMetadata | undefined> {
      try {
          const db = await this.getDB();
          const tx = db.transaction(['static_books', 'static_book_sources', 'user_book_states', 'cache_covers'], 'readonly');
  ...
  async updateBookMetadata(id: string, metadata: Partial<BookMetadata>): Promise<void> {
    try {
      const db = await this.getDB();
      const tx = db.transaction(['static_books', 'static_book_sources', 'user_book_states', 'cache_covers'], 'readwrite');
      ...
      let coverUpdate: Blob | undefined;

      const bookKeys: (keyof Book)[] = ['title', 'author', 'description', 'coverUrl', 'addedAt'];
      ...
      if (coverUpdate) {
          await tx.objectStore('cache_covers').put(coverUpdate, id);
      }
  ...
  async getCachedSegment(key: string): Promise<CachedSegment | undefined> {
      try {
          const db = await this.getDB();
          const segment = await db.get('cache_tts_audio', key);

          if (segment) {
              // Fire and forget update to lastAccessed
              // We don't await this to keep read fast
              db.put('cache_tts_audio', { ...segment, lastAccessed: Date.now() }).catch((err) => Logger.error('DBService', 'Failed to update TTS cache lastAccessed', err));
          }
          return segment;
      } catch (error) {
          this.handleError(error);
  ...
  async cacheSegment(key: string, audio: ArrayBuffer, alignment?: any[]): Promise<void> {
      try {
          ...
          await db.put('cache_tts_audio', segment);
      } catch (error) {
          this.handleError(error);
      }
  }
  ...
  async getLocations(bookId: string): Promise<BookLocations | undefined> {
      try {
          const db = await this.getDB();
          return await db.get('cache_locations', bookId);
      } catch (error) {
          this.handleError(error);
      }
  }
  ...
  async saveLocations(bookId: string, locations: string): Promise<void> {
      try {
          const db = await this.getDB();
          await db.put('cache_locations', { bookId, locations });
      } catch (error) {
          this.handleError(error);
      }
  }
