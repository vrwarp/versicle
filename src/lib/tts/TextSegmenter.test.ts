...
describe('TextSegmenter', () => {
  ...
  it('allows custom abbreviations via configuration', () => {
    // "Fig." is not in defaults.
    const text = "See Fig. 1 for details.";

    // Default behavior splits at "Fig."
    const defaultSegmenter = new TextSegmenter();
    const defaultSegments = defaultSegmenter.segment(text);
    // Intl.Segmenter might handle Fig. natively depending on locale/version,
    // but assuming it might split or we want to force it not to.
    // Actually, "Fig." often splits.

    // Custom behavior
    const customSegmenter = new TextSegmenter({
        locale: 'en',
        additionalAbbreviations: ['Fig.']
    });
    const customSegments = customSegmenter.segment(text);

    expect(customSegments).toHaveLength(1);
    expect(customSegments[0].text).toBe("See Fig. 1 for details.");
  });
