...
/** A seeded cache_tts_preparation sentence row (dbService.getTTSContent). */
...
/** A seeded section row (dbService.getSections). */
...
export interface ParityHarness {
    ...
    host: {
        ...
        /** Make dbService.getTTSContent REJECT for this section (an unloadable section). */
        failTTSContent(bookId: string, sectionId: string): void;
        /** Defer dbService.getSections(bookId) until the returned release fn is called (P18). */
        ...
        /** Calls to dbService.getTTSContent for this section (P18 staleness observable). */
