 ...
 * host ports for async reads (book metadata, persisted ContentAnalysis rows). The dbService
 * the worker-resident engine imports is the same mocked module (the engine-dir vi.mock
 * allowlist: {@db/DBService, ../LexiconService} here — PlatformIntegration is proxied, not
 * imported, on this transport).
 */
