...
export class BackupService {
  ...
  private async processManifest(
    ...
    const tx = db.transaction(['static_books', 'static_book_sources', 'user_book_states', 'user_annotations', 'cache_locations', 'user_lexicon'], 'readwrite');
    ...
    for (const loc of locations) {
        await tx.objectStore('cache_locations').put(loc);
