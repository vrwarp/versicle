...
describe('SearchClient Race Condition', () => {
    ...
    it('should handle concurrent searches correctly', async () => {
        ...
        try {
             // Because of the out-of-order tracking fix, p1 will be rejected with "Search superseded"
             // Wait for both to finish (either resolved or rejected)
             // eslint-disable-next-line @typescript-eslint/no-explicit-any
             const results = await Promise.race([Promise.allSettled([p1, p2]), timeout]) as any[];

             // Verify correct mapping
             expect(results[0].status).toBe('rejected');
             expect(results[0].reason.message).toBe('Search superseded');
