...
export class AndroidBackupService {
    ...
    static async writeBackupPayload(manifest: SyncManifest): Promise<void> {
        try {
            ...
            Logger.error('AndroidBackupService', 'Failed to write Android backup payload', e);
        }
    }
    ...
    static async readBackupPayload(): Promise<SyncManifest | null> {
        try {
            ...
            Logger.warn('AndroidBackupService', 'No backup payload found or failed to read', e);
            return null;
        }
    }
}
