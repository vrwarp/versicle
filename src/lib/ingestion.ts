...
export async function reprocessBook(bookId: string): Promise<void> {
  ...
  // Transaction
  const tx = db.transaction(['static_book_sources', 'static_sections', 'static_tts_content', 'cache_table_images'], 'readwrite');

  // Helper to delete by index
  const deleteByIndex = async (storeName: 'static_sections' | 'static_tts_content' | 'cache_table_images') => {
  ...
  await deleteByIndex('cache_table_images');
...
export async function processEpub(
  ...
  const bookData: Book = {
    ...
    addedAt: Date.now(),
    // coverBlob is now stored separately
  };
  ...
  const finalCover = thumbnailBlob || coverBlob;
  if (finalCover) {
      await tx.objectStore('cache_covers').add(finalCover, bookId);
  }
