...
export const useAllBooks = () => {
    ...
    // BOLT OPTIMIZATION: Pre-compute match keys for reading list entries in a separate useMemo
    // to avoid O(N * M) string parsing (generateMatchKey) inside the frequently-running progress map loop.
    // readingListEntries changes rarely, whereas progressMap changes on every page turn.
