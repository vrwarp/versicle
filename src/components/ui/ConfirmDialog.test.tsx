...
describe('ConfirmDialog (useConfirm / confirmDialog)', () => {
  ...
  it('resolves false on cancel', async () => {
    render(<ConfirmHost />);
    const promise = confirmDialog({ titleKey: 'data.clearAll.title', bodyKey: 'data.clearAll.body' });
  ...
  it('danger requests default the confirm label to Delete; plain ones to Confirm', async () => {
    ...
    const dangerous = confirmDialog({ titleKey: 'data.clearAll.title', danger: true });
    expect(await screen.findByTestId('confirm-dialog-confirm')).toHaveTextContent('Delete');
    fireEvent.click(screen.getByTestId('confirm-dialog-cancel'));
    await dangerous;

    const plain = confirmDialog({ titleKey: 'data.regenerate.title' });
    await waitFor(() =>
  ...
  it('queues requests: the second dialog appears after the first settles', async () => {
    ...
    const first = confirmDialog({ titleKey: 'data.clearAll.title' });
    const second = confirmDialog({ titleKey: 'data.regenerate.title' });
  ...
  it('a request made BEFORE any host mounts renders once a host appears (boot-time parity with the toast queue)', async () => {
    const promise = confirmDialog({ titleKey: 'app.resetAll.title', bodyKey: 'app.resetAll.body' });
