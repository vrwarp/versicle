...
export const GlobalSettingsDialog = () => {
    ...
    const { currentTheme, setTheme } = useReaderSyncStore(useShallow(state => ({
        currentTheme: state.currentTheme,
        setTheme: state.setTheme
    })));

    const { isSyncEnabled, setSyncEnabled } = useSyncStore();
            ...
            <Modal open={isGlobalSettingsOpen} onOpenChange={setGlobalSettingsOpen}>
                <ModalContent className="max-w-3xl h-[90vh] sm:h-[600px] flex flex-col sm:flex-row p-0 overflow-hidden gap-0 sm:rounded-lg" aria-describedby="global-settings-desc">
                    ...
                    <div className="w-full sm:w-3/4 p-4 sm:p-8 overflow-y-auto flex-1">
                        ...
                        {activeTab === 'sync' && (
                            <div className="space-y-6">
                                <h3 className="text-lg font-medium mb-4">Sync & Backup</h3>
                                <div className="space-y-4">
                                    <div className="space-y-2">
                        ...
                        {activeTab === 'recovery' && (
                            <div className="space-y-6">
                                <h3 className="text-lg font-medium mb-4">Recovery Checkpoints</h3>
                                <div className="space-y-2">
                                    {recoveryStatus && <p className="text-sm text-muted-foreground">{recoveryStatus}</p>}
                                    <div className="border rounded-md divide-y max-h-[400px] overflow-y-auto">
                                        {checkpoints.length === 0 ? (
                                            <div className="p-4 text-sm text-muted-foreground text-center">No checkpoints found</div>
                                        ) : (
                                            checkpoints.map(cp => (
                                                <div key={cp.id} className="p-3 flex items-center justify-between">
                                                    <div>
                                                        <div className="font-medium text-sm">Checkpoint #{cp.id}</div>
                                                        <div className="text-xs text-muted-foreground">{new Date(cp.timestamp).toLocaleString()}</div>
                                                        <div className="text-xs text-muted-foreground">
                                                            Books: {Object.keys(cp.manifest.books).length},
                                                            RL: {Object.keys(cp.manifest.readingList).length},
                                                            Anns: {Object.values(cp.manifest.books).reduce((acc, b) => acc + (b.annotations?.length || 0), 0)}
                                                        </div>
                                                    </div>
                        ...
                        {activeTab === 'data' && (
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <Button variant="outline" className="w-full" onClick={handleRepairDB} disabled={isScanning}>
                                        {isScanning ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                                        Repair Database (Find Orphans)
                                    </Button>
                                    {orphanScanResult && <p className="text-xs text-muted-foreground p-2 bg-muted rounded">{orphanScanResult}</p>}

                                    <Button variant="outline" className="w-full" onClick={handleExportFull}>
                                        Create Full Backup (ZIP)
                                    </Button>
                                    {backupStatus && <p className="text-xs text-muted-foreground p-2 bg-muted rounded">{backupStatus}</p>}
                                    ...
                                    <div className="pt-4 border-t">
                                        <label className="text-sm font-medium block mb-2">Reading List (CSV) <span className="text-muted-foreground font-normal">({readingListCount} items)</span></label>
                                        <div className="flex gap-2">
                                            <Button
                                                variant="outline"
                                                className="flex-1"
                                            ...
                                            <Button
                                                variant="outline"
                                                className="flex-1"
                                                onClick={handleImportReadingListClick}
                                            >
                                                Import CSV
                                            </Button>
                                            <input
