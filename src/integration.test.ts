...
vi.mock('./lib/ingestion', () => ({
    processEpub: vi.fn(async (file: File) => {
        ...
        await db.put('cache_covers', new Blob(['mock-cover'], { type: 'image/jpeg' }), bookId);
        await db.put('static_book_sources', {
            bookId,
            fileHash: 'mock-hash'
        });
        await db.put('user_book_states', {
            bookId,
            progress: 0,
            isOffloaded: false
        });
...
describe('Feature Integration Tests', () => {
  ...
  beforeEach(async () => {
    // Clear DB
    const db = await getDB();
    const tx = db.transaction(['static_books', 'static_book_sources', 'user_book_states', 'static_files', 'user_annotations', 'static_sections', 'static_tts_content', 'cache_covers'], 'readwrite');
  ...
  it('should track reading progress and persist it', async () => {
      ...
      // Simulate the persistence logic used in ReaderView (or via DBService)
      const saveProgress = async (id: string, cfi: string, prog: number) => {
        ...
        const state = await store.get(id) || { bookId: id };
