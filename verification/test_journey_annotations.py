...
def test_annotations_journey(page: Page):
    ...
    # 3. Create Green Highlight (Test for Bug Fix)
    print("Creating Green Highlight...")

    # Inject script to select text
    selection_success = frame.locator("body").evaluate("""
        () => {
            try {
                const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, null, false);
                let node = walker.nextNode();
                // Find a decent chunk of text later in the doc
                while(node) {
                ...
                if (node) {
                    ...
                    // Trigger mouseup to show popover
                    document.dispatchEvent(new MouseEvent('mouseup', {
                        view: window,
                        bubbles: true,
                        cancelable: true,
    ...
    if not selection_success:
        print("Could not select text for green highlighting.")
        utils.capture_screenshot(page, "annotations_failed_selection_green")
        return

    # Click Green
    expect(page.get_by_test_id("popover-color-green")).to_be_visible(timeout=2000)
    page.get_by_test_id("popover-color-green").click()
    expect(page.get_by_test_id("popover-color-green")).not_to_be_visible()

    # Wait for highlight to be applied
    page.wait_for_timeout(1000)

    # 4. Verify Green Color
    print("Verifying Green Highlight Visual Color...")
    color_check = frame.locator("body").evaluate("""
        () => {
            // Find highlight with class 'highlight-green'
            const highlights = Array.from(document.querySelectorAll('.highlight-green'));
            if (highlights.length === 0) return { found: false };
    ...
    print(f"Green Highlight Check: {color_check}")

    if not color_check['found']:
        pytest.fail("No element with class 'highlight-green' found.")

    fill = color_check['fill']
    # Check if it's yellow (the bug)
    if 'rgb(255, 255, 0)' in fill:
         pytest.fail(f"Regression: Green highlight rendered as YELLOW ({fill})!")
