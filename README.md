...
*   **Worker TTS**: Background TTS processing (e.g. WASM inference) decoupled from the main thread UI to maximize performance and avoid UI freezes. The monolithic `AudioPlayerService` has been replaced by an isolated `PlaybackController` (`src/lib/tts/engine/`) using `EngineContext` for Web Worker portability without jsdom.
