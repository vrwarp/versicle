...
| --- | --- | --- | --- | --- |
| C1 | IndexedDB storage schema | `src/data/schema.ts` | zod rows in src/data/rows/; append-only versioned migration registry (DB v31) | `src/data/migrations.test.ts`, `src/data/connection.test.ts`, `src/data/__fixtures__/schema-fixtures.ts` |
...
error/0), lint-debt allowlist **NaN** `any`-sites +
**41** disables (`lint-debt-allowlist.json`), coverage
...
The IndexedDB schema (`EpubLibraryDB`) is at **v31** (`DB_VERSION`,
...
| 2 | `openDB` | open EpubLibraryDB through the versioned migration registry (v31) |
