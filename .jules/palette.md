...
## 2026-04-10 - Keyboard Accessibility for JsonDiffViewer
...
## 2026-04-10 - Standardizing Native Buttons in Complex Components
**Learning:** Found multiple instances of native `<button>` tags being used in the `ContentAnalysisLegend` component for various interactions (accordions, actions, copy buttons). These lacked the standard focus visibility and hover states provided by the design system, causing accessibility regressions for keyboard users.
**Action:** Always replace native HTML `<button>` elements with the shared `<Button>` component (`src/components/ui/Button.tsx`). For unstyled or custom layouts, leverage the `variant="ghost"`, `size="sm"`, or `size="icon"` props combined with custom class names (e.g., `h-auto p-0 hover:bg-transparent`) to preserve the visual design while inheriting necessary accessibility and standard focus features.
