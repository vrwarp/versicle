...
## 2026-05-05 - [Icon-Only Native Buttons vs Design System Buttons]
**Learning:** When using icon-only buttons for actions like accordion toggles or sort actions within lists/tables, native `<button>` elements often lack accessible names or standard design system hover/focus states. Furthermore, simply attaching an `aria-label` to a raw `<button>` might fix screen reader context but doesn't resolve styling inconsistencies.
**Action:** Always replace native icon-only `<button>` tags with the `<Button variant="ghost" size="icon">` component from the design system, and critically, attach a descriptive `aria-label` attribute (or adapt it based on state like `isExpanded`) to ensure assistive technologies can correctly interpret the action regardless of the visual icon.
