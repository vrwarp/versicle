...
## 2026-04-26 - Keyboard Accessibility for JsonDiffViewer
...
## 2026-05-04 - [Loading States for Async Buttons]
**Learning:** For interactive UI elements acting as custom buttons (like `div` with `role="button"`), applying the native `disabled` attribute does not actually disable the element in HTML, leading to potential multiple-click regressions. Using `aria-disabled` combined with explicit conditional click handlers is required. Furthermore, adding `opacity-80` to loading action components provides a subtle but effective visual cue that the element is temporarily out of service without the heavy dimming of standard disabled styles, which works well in reading environments.
**Action:** When adding loading states to custom interactive elements, always implement both `aria-disabled={isLoading}` and a conditional check in the `onClick` and `onKeyDown` handlers (e.g., `if (isLoading) return;`). Consider using `opacity-80` or `active:scale-100` alongside `cursor-wait` for better micro-interactions.
