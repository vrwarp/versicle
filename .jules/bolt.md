...
## 2026-03-09 - [Selector Tuning: Replacing Object.values]
**Learning:** `Object.values()` creates a new array instance every time it is called. When used in fast-path selectors (like inside `src/store/selectors.ts` or `src/store/useLibraryStore.ts` during map iterations), it can cause memory bloat and degrade performance during rapid state updates (e.g. library sorting or syncing). However, completely replacing it with manual `for...in` loops *everywhere* degrades code readability and can be counterproductive if the JS engine already optimizes `Object.values()` well.
**Action:** Only optimize `Object.values()` to `for...in` inside critical *fast-path* Zustand selectors (like `useAllBooks` which runs repeatedly during page turns) or tightly nested loops handling thousands of items. Do *not* apply this optimization to simple React UI components where the array size is small and readability is more important than microsecond gains.
