...
- **Solution:** Created a new `getBooksMetadata(ids: string[])` method in `DBService` that executes `Promise.all(manifestStore.get(...))` within a single `readonly` transaction scope. Updated the library store to use this batched method.
- **Learning:** Never launch dynamically scaled `O(N)` independent database transactions during startup or hot paths. Always use a batched accessor that collects all `get` requests and resolves them under a single transaction to avoid IDB queue overhead.
