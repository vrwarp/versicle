...
if [[ "$1" == "--help" ]]; then
  ...
  echo "     (npm ci, VITE_E2E=true npm run build, Playwright browsers preinstalled)."
  echo "  2. Runs a container (--ipc=host, screenshots dir mounted) which:"
  echo "     a. Starts the app in preview mode ('npm run preview')."
  echo "     b. Waits for the app to become available at http://localhost:5173."
  echo "     c. Executes 'npx playwright test' with the provided arguments."
  echo ""
  echo "Arguments:"
  echo "  Apart from --logs/--probe (handled here), all arguments are forwarded"
  echo "  directly to 'npx playwright test' inside the container. The suite is"
  ...
  echo "      ./run_verification.sh verification/test_journey_library.spec.ts"
  echo ""
  echo "  - Run a single project:"
  echo "      ./run_verification.sh --project=webkit"
  echo ""
  echo "  - Run tests matching a title (playwright --grep):"
  echo "      ./run_verification.sh --grep 'search'"
  echo ""
  echo "  - Disable parallel execution (useful for debugging):"
  echo "      ./run_verification.sh --workers=1"
  echo ""
  ...
  echo ""
  echo "  - Enable the IndexedDB/event-loop hang probe (sets TTS_IDB_PROBE=1):"
  echo "      ./run_verification.sh --probe verification/test_tts_queue.spec.ts"
  echo ""
  echo "Artifacts:"
  ...
  echo "  - If you are Jules (or docker needs root), use the 'jules_run_verification.sh' wrapper."
  ...
  echo "  - TESTING.md (repo root) is the canonical testing document."
  exit 0
fi
